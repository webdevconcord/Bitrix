...
if (CModule::IncludeModule('sale')) {
    ...
    if (! empty($orderID)) {
        ...
        include $_SERVER['DOCUMENT_ROOT'] . '/bitrix/php_interface/include/sale_payment/concordpay_payment/concordpay.php';
        ...
        $currency = empty(CSalePaySystemAction::GetParamValue('CP_PRICE_CURRENCY')) ?
            Concordpay::CURRENCY_HRYVNA :
            CSalePaySystemAction::GetParamValue('CP_PRICE_CURRENCY');

        if ($merchantID !== $data['merchantAccount']) {
            $errorMessages .= Loc::GetMessage('CONCORDPAY_ERROR_MERCHANT') . PHP_EOL;
        }

        if ($currency !== $data['currency']) {
            $errorMessages .= Loc::GetMessage('CONCORDPAY_ERROR_CURRENCY') . PHP_EOL;
        }

        if ((float)$arOrder['PRICE'] !== (float)$data['amount']) {
            $errorMessages .= Loc::GetMessage('CONCORDPAY_ERROR_AMOUNT') . PHP_EOL;
        }

        $concordpayPayment = new Concordpay();
        $CPResult = $concordpayPayment->isPaymentValid($data);
    } else {
        $CPResult = Loc::GetMessage('CONCORDPAY_ERROR_ORDER');
    }
